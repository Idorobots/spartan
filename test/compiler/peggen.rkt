...
(describe
             ...
             '(let ((result1 (Foo in off)))
                (if (matches? result1)
                    (let ((end2 (match-end result1)))
                      (let ((result3 (Bar in end2)))
                        (if (matches? result3)
                            (let ((end4 (match-end result3)))
                              (let ((result5 (Baz in end4)))
                                (if (matches? result5)
                                    (let ((end6 (match-end result5)))
                                      (matches (list (match-match result1)
                                                     (match-match result3)
                                                     (match-match result5))
                                               (match-start result1)
                                               end6))
     ...
     (assert (generate-zero-or-more '(* Foo) 'in 'off id)
             '(let loop4 ((results3 '())
                ...
                (let ((result1 (Foo in end2)))
                  (if (matches? result1)
                      (loop4 (cons result1 results3)
                             (match-end result1))
                      (let ((final-result5 (reverse results3)))
                        (matches (map match-match final-result5)
                                 (if (empty? final-result5)
                                     off
                                     (match-start (car final-result5)))
                                 end2))))))
     (gensym-reset!)
 ...
 (it "(+ ...) matches at least one value"
     ...
     (assert (generate-one-or-more '(+ Foo) 'in 'off id)
             '(let ((result6 (let loop4 ((results3 '())
                                         (end2 off))
                               (let ((result1 (Foo in end2)))
                                 (if (matches? result1)
                                     (loop4 (cons result1 results3)
                                     ...
                                     (let ((final-result5 (reverse results3)))
                                       (matches (map match-match final-result5)
                                                (if (empty? final-result5)
                                                    ...
                                                    (match-start (car final-result5)))
                                                end2)))))))
                (if (and (matches? result6)
                         (= 0 (length (match-match result6))))
                    (no-match)
                    result6)))
     (gensym-reset!)
