...
;; Can step a process:
(assert (not (executable? (uproc 100 nil 0 'waiting))))
(assert (executable? (uproc 100 (&yield-cont id nil) 0 'waiting)))
(assert (uproc? (execute-step! (uproc 100 (&yield-cont id nil) 0 'waiting))))

;; Can modify task list:
(let ((t (uproc 100 nil 0 'waiting)))
  (assert (do (reset-tasks! (list t))
              (next-task))
          t))

(let* ((t1 (uproc 100 nil 1 'waiting))
       (t2 (uproc 100 nil 2 'waiting)))
