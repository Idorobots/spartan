  ...
  (let ((n (flip normalize id)))
    (cond ((symbol? expr) (kont expr))
          ...
          ((fix? expr) (kont (make-fix (map (lambda (b)
                                                  ...
                                                  (fix-bindings expr))
                                             (n (fix-body expr)))))
          ((letcc? expr) (kont (make-letcc (letcc-var expr)
                                           (n (letcc-body expr)))))
          ((reset? expr) (kont (make-reset (n (reset-expr expr)))))
          ((shift? expr) (kont (make-shift (shift-cont expr)
                                           (n (shift-expr expr)))))
          ((handle? expr) (normalize-named (handle-handler expr)
                                           (lambda (h)
                                             (kont (make-handle (n (handle-expr expr))
                                                                h)))))
          ((raise? expr) (normalize-named (raise-expr expr)
                                          (lambda (e)
                                            (kont (make-raise e)))))
