...
(define (traverse-ast f acc expr)
  (match-ast expr
   ...
   ((lambda formals body)
    (set-ast-node-data expr (ast-lambda-data (traverse-ast-map f acc formals)
                                             (f acc body))))
   ((let bindings body)
    (set-ast-node-data expr (ast-let-data (traverse-ast-map f acc bindings)
                                          (f acc body))))
   ((letrec bindings body)
    (set-ast-node-data expr (ast-letrec-data (traverse-ast-map f acc bindings)
                                             (f acc body))))
   ((fix bindings body)
    (set-ast-node-data expr (ast-fix-data (traverse-ast-map f acc bindings)
                                          (f acc body))))
   ((binding var val)
    (set-ast-node-data expr (ast-binding-data (f acc var)
                                              (f acc val)
   ...
   ((def name value)
    (set-ast-node-data expr (ast-def-data (f acc name)
                                          (f acc value))))
   ((app op args ...)
    (set-ast-node-data expr (ast-app-data (f acc op)
                                          (traverse-ast-map f acc args))))
   ((primop-app op args ...)
    (set-ast-node-data expr (ast-primop-app-data op (traverse-ast-map f acc args))))
   ((list exprs ...)
